...
$(function(){
	...
	$(".purchase-num-add").click(function(){
		...
		for (var i=0;i < len; i++) {
			var price = $('.check-all-xz:eq('+ i +')').parents('.goods-list-item').find('.goods-dl-price span').text();
	...
	$('#checkAll').on('click',function(){
		...
		if($(this).hasClass('check-all-xz')){
			...
			for (var i=0;i < len; i++) {
				var price = $('.goods-list-item').eq(i).find('.goods-dl-price span').text();
	...
	$('.checkone').on('click',function(){
		...
		if($(this).hasClass('check-all-xz')){
			...
			//计算总金额展示
			var price = $(this).parents('.goods-list-item').find('.goods-dl-price span').text();
