...
(function ($) {
    $.fn.date = function (options, Ycallback, Ncallback) {
        ...
        //当月天数
        var sumDay = getSumDay(nowdate.getFullYear(), initM);
        ...
        function refreshTime() {
            ...
            if (initH > 12) {    //判断当前时间是上午还是下午
                SecondScroll.scrollTo(0, initD * 43 - 43, 100, true);   //显示“下午”
                initH = initH - 12 - 1;
            }
            HourScroll.scrollTo(0, initH * 43, 100, true);
            MinuteScroll.scrollTo(0, initI * 43, 100, true);
...
//              Ncallback(false);
        ...
        function init_iScrll() {
            ...
            monthScroll = new iScroll("monthwrapper", {
                snap: "li", vScrollbar: false,
                onScrollEnd: function () {
                    indexM = (this.y / 43) * (-1) + 2;
                    $("#monthwrapper ul li").removeClass("on");
                    ...
                    $("#daywrapper ul li:eq(" + Math.abs(Math.round(indexD)) + ")").addClass("on");
                }
            });
            dayScroll = new iScroll("daywrapper", {
                snap: "li", vScrollbar: false,
                onScrollEnd: function () {
                    indexD = (this.y / 43) * (-1) + 2;
                    $("#daywrapper ul li").removeClass("on");
                    $("#daywrapper ul li:eq(" + Math.abs(Math.round(indexD)) + ")").addClass("on");
                }
            });
        ...
        function init_iScroll_datetime() {
            HourScroll = new iScroll("Hourwrapper", {
                snap: "li", vScrollbar: false,
                onScrollEnd: function () {
                    indexH = Math.round((this.y / 43) * (-1)) + 1;
                    HourScroll.refresh();
                }
            })
            MinuteScroll = new iScroll("Minutewrapper", {
                snap: "li", vScrollbar: false,
                onScrollEnd: function () {
                    indexI = Math.round((this.y / 43) * (-1)) + 1;
                    HourScroll.refresh();
                }
            })
